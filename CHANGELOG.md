...
- Входная диагностика удалена из фазы 00: все студенты проходят общее ядро в одном
  ...
  проверки готовности. Фаза 00 теперь состоит из пяти уроков.
- Урок `00/02` о терминале сокращён до необходимого минимума: навигация, безопасные
...
- Все 200 уроков теперь публикуются как самостоятельные статические страницы сайта:
...
- Контракт урока разделяет `executable` и `guided-artifact` practice: код обязателен,
  ...
  Первый урок переработан в сценарную практику выбора маршрута с заполняемым планом,
  отдельной рубрикой и квизом на перенос вместо искусственного классификатора.
