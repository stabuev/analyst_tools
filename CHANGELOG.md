...
- После сквозного аудита фазы 04 исправлены устаревшие ссылки между уроками, добавлен
  мост `pandas.concat` → `UNION ALL`, а финальный `04/13` получил компактную phase-exit
  самопроверку окон, frames, когорт и plan shape без отдельного submission harness.
  Длительность насыщенного `04/08` увеличена до 120 минут; SQL-фаза теперь честно
  оценивается в 20,5 часа.
- Входная диагностика удалена из фазы 00: все студенты проходят общее ядро в одном
  порядке, а подготовительные материалы остаются необязательным справочником без
  проверки готовности.
- Карта профессий вынесена из нумерованной программы в подробное ненумерованное введение:
  выбор маршрута больше не требует теста и обязательного артефакта. Фаза 00 теперь
  начинается с терминала и состоит из четырёх уроков; страница маршрутов сайта показывает
  похожие профессии и рабочий результат каждого направления.
- Из фазы 01 удалены самостоятельные уроки про версии Python и `pyproject.toml`.
  Необходимый минимум про версию встроен в урок `01/01` о `uv`, а секции manifest
  вводятся по мере необходимости в уроках про package, Ruff и pytest. Фаза состоит из
  семи последовательных уроков.
- Урок `00/01` о терминале сокращён до необходимого минимума: навигация, безопасные
  операции с файлами, простые потоки и короткий Bash-скрипт вместо production-grade CLI;
  дополнительное чтение расширено до десяти русскоязычных и англоязычных материалов.
- Все 197 уроков теперь публикуются как самостоятельные статические страницы сайта:
  полный учебный текст, pre/post-квиз, проверяемая практика, артефакты, прогресс и
  навигация между уроками доступны без обязательного перехода на GitHub. Для
  исполняемых уроков публикуются тесты и reference-код, для направляемых — шаблон и
  рубрика. Sitemap и structured data расширены до каждой страницы урока.
- Контракт урока разделяет `executable` и `guided-artifact` practice: код обязателен,
  только если программный или вычислительный механизм входит в результат обучения;
  направляемая практика проверяется самостоятельным шаблоном и рубрикой.
- Сайт подготовлен к поисковой индексации: добавлены canonical и social metadata,
  JSON-LD курса, sitemap, robots policy, favicon и 404; каталог, маршруты и глоссарий
  теперь присутствуют в исходном HTML и не требуют JavaScript для чтения crawler-ом.
- Добавлен путь новичка от установки Git и uv до первого запуска, отдельные бесплатные
  маршруты подготовки по Python, SQL и статистике и явный самостоятельный цикл работы
  над каждым уроком.
- Добавлен интерактивный phase quiz runner с перемешиванием вопросов и вариантов,
  отложенной обратной связью и сохраняемым отчетом попытки; контракт новых квизов
  усилен требованиями к правдоподобным дистракторам.
- Фазы 03 и 07 получили сквозные учебные путеводители с единым проектным scaffold,
  промежуточными контрольными точками и критериями готовности.
- Добавлены маршрут «Аналитик временных рядов», независимое review капстоуна без
  платной платформы, уточненная политика шкал визуализации и расширенный глоссарий.
- Сайт теперь объясняет самостоятельный учебный цикл и явно отделяет локальную отметку
  прогресса от проверки практической работы.

### Исправлено

- Checksum-цепочка капстоуна `18/04`–`18/06` синхронизирована с текущим `uv.lock`,
  поэтому полный lesson runner снова оставляет рабочее дерево неизменным.
...
- Фаза 18 «Капстоун-проекты»: семь stage-gated уроков от problem brief и data contract
  ...
  завершен полностью: 19 фаз и 197 из 197 уроков в текущей программе.
