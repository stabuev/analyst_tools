...
class CourseStructureTest(TestCase):
    ...
    def test_site_has_indexable_static_content(self) -> None:
        ...
        expected_lessons = sum(
            len(phase["lessons"]) for phase in load_curriculum()["phases"]
        )
        self.assertEqual(
            catalog.count('<tr><td><span class="phase-chip">'), expected_lessons
        )
    ...
    def test_sitemap_and_404_indexing_policy(self) -> None:
        ...
        self.assertEqual(len(lesson_urls), expected_lessons)
    ...
    def test_lesson_pages_embed_course_content_and_practice(self) -> None:
        ...
        self.assertEqual(len(canonicals), expected_lessons)
    ...
    def test_only_completed_lessons_create_links(self) -> None:
        ...
        self.assertIn("phases/00-entry-and-tools/01-terminal-and-filesystem", roadmap)
        self.assertIn("phases/00-entry-and-tools/02-git-foundations", roadmap)
        self.assertIn("phases/00-entry-and-tools/03-branches-and-review", roadmap)
        self.assertIn("phases/00-entry-and-tools/04-secrets-and-sensitive-data", roadmap)
        self.assertIn("phases/01-reproducible-project/01-uv-environments", roadmap)
        self.assertIn("phases/01-reproducible-project/02-jupyter-kernels", roadmap)
        self.assertIn(
            "phases/01-reproducible-project/03-notebook-reproducibility",
            roadmap,
        )
        self.assertIn(
            "phases/01-reproducible-project/04-modules-and-scripts",
            roadmap,
        )
        self.assertIn("phases/01-reproducible-project/05-ruff", roadmap)
        self.assertIn("phases/01-reproducible-project/06-pytest", roadmap)
        self.assertIn(
            "phases/01-reproducible-project/07-continuous-integration",
            roadmap,
        )
