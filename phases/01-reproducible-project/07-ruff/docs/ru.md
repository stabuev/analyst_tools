...
**Тип:** Build
**Треки:** Core
**Пререквизиты:** 01/06 — От ноутбука к модулям и скриптам
**Время:** ~45 минут
...
```bash
python3 -m unittest discover \
  -s phases/01-reproducible-project/07-ruff/tests \
  -p "test_*.py" -v
```
...
```bash
python3 phases/01-reproducible-project/07-ruff/code/main.py
```
