...
**Тип:** Build
**Треки:** Core
**Пререквизиты:** 01/03 — Воспроизводимые ноутбуки
**Время:** ~75 минут
...
### Ровно те части `pyproject.toml`, которые нужны package

В уроке про `uv` мы использовали `[project]` для версии Python и зависимостей. Теперь
проект впервые нужно установить как package, поэтому появляются ещё две части:
...
`[build-system]` отвечает на узкий вопрос: каким инструментом собрать устанавливаемый
package. `[project]` по-прежнему описывает сам проект. Остальные секции не нужно добавлять
заранее: Ruff и pytest внесут свои настройки в тех уроках, где они действительно
понадобятся.
...
python3 -m unittest discover \
  -s phases/01-reproducible-project/04-modules-and-scripts/tests \
  -p "test_*.py" -v
```
...
```bash
python3 phases/01-reproducible-project/04-modules-and-scripts/code/main.py
```
...
python3 -m compileall \
  phases/01-reproducible-project/04-modules-and-scripts/outputs/order_metrics_project/src
```
