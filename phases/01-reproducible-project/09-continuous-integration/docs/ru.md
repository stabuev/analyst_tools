...
**Тип:** Build
**Треки:** Core
**Пререквизиты:** 01/08 — Первые проверки с pytest
**Время:** ~60 минут
...
python3 -m unittest discover \
  -s phases/01-reproducible-project/09-continuous-integration/tests \
  -p "test_*.py" -v
```
...
```bash
python3 phases/01-reproducible-project/09-continuous-integration/code/main.py
```

Полный локальный эквивалент:

```bash
cd phases/01-reproducible-project/09-continuous-integration/outputs/ci_project
uv sync --locked --dev
