...
**Тип:** Build
**Треки:** Core
**Пререквизиты:** 01/03 — pyproject.toml как контракт проекта
**Время:** ~60 минут
...
python3 -m unittest discover \
  -s phases/01-reproducible-project/04-jupyter-kernels/tests \
  -p "test_*.py" -v
```
...
```bash
python3 phases/01-reproducible-project/04-jupyter-kernels/code/main.py
```

Проверьте реальный локальный kernelspec:

```bash
python3 phases/01-reproducible-project/04-jupyter-kernels/outputs/kernel_diagnostic.py \
