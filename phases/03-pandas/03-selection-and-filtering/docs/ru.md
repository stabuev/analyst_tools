...
систематически разобрана в `03/10`.

После `03/10` тот же селектор можно применять и к `category`, если категории являются
уже нормализованными строковыми значениями. Он сохраняет missing как `pd.NA` и выполняет
точное сравнение со словарём; категориальный столбец с числовыми уровнями отклоняется как
другой предметный контракт. Нормализация по-прежнему не прячется внутри фильтра.
...
- строковую нормализацию и категории до `03/10`;
...
- `build_order_mask` строит nullable-маску только из уже проверенных числовых,
  строковых или строковых категориальных столбцов;
