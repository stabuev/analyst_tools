...
select
    ...
    {{ to_decimal('coalesce(sum(orders.gross_revenue_rub), 0)') }} as gross_revenue_rub,
    {{ to_decimal('coalesce(sum(orders.paid_revenue_rub), 0)') }} as paid_revenue_rub,
    {{ to_decimal('coalesce(sum(orders.refunded_amount_rub), 0)') }} as refunded_amount_rub,
