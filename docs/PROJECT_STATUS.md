...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `2e35fa2e` — карта профессий и контракт чтения

Все 200 уроков курса завершены. Onboarding, подготовительные материалы, самостоятельный
учебный цикл, phase quiz runner, путеводители по фазам 03 и 07, маршрут временных рядов,
независимое review капстоуна, визуальная политика, глоссарий и SEO-контракт зафиксированы
в `1e56f11`. Публичный сайт размещён на
`https://datascience.xyz/courses/analyst-tools/`.

В текущем незакоммиченном этапе сайт переводится с GitHub-ссылок на самостоятельные
страницы всех 200 уроков. Каждая страница содержит полный Markdown-текст, pre/post-квиз,
проверяемую практику, артефакты, прогресс, соседние уроки, canonical и JSON-LD. Для
`executable`-уроков страница показывает тесты и reference-код, для `guided-artifact` —
самостоятельный шаблон и рубрику. Sitemap содержит четыре служебные страницы и 200
страницу урока. Structural validator, render-check и 45 корневых тестов проходят.
Пользовательский `site/Archive.zip` не входит в генерацию и должен оставаться отдельным
от изменений курса.

Контракт урока теперь выбирает форму практики по наблюдаемому результату. Режим
`executable` остается обратимо совместимым значением по умолчанию и требует
`code/main.py` с behavioral tests. Режим `guided-artifact` требует отдельный заполняемый
артефакт и рубрику, поэтому код не является формальным условием завершения. Урок `00/01`
переведен в этот режим: вместо route classifier студент разбирает рабочую ситуацию,
обосновывает основной и дополнительный маршруты курса и проверяет план по рубрике.
Контракт дополнительного чтения допускает от трех до десяти отобранных и аннотированных
ссылок. В `00/01` собраны десять русскоязычных и англоязычных материалов по направлениям
аналитической работы без ссылок на Яндекс Практикум.
Входная диагностика удалена из курса: все студенты проходят общее ядро последовательно,
без проверки готовности и выбора стартовой фазы. Необязательные материалы по базовому
Python, SQL и статистике сохранены как справочник, к которому обращаются по необходимости.
Урок `00/02` «Терминал и файловая система» перестроен вокруг минимального результата:
навигации, безопасных операций с файлами, базовых потоков и самостоятельно собранного
короткого `folder_summary.sh`. Продвинутые Bash-механизмы исключены из обязательной части,
а дополнительное чтение содержит десять отобранных материалов на русском и английском.
...
Готовность по фазам: `00` — 5/5, `01` — 9/9, `02` — 9/9, `03` — 11/11,
...
Все 19 фаз и 200 уроков завершены. Следующий содержательный шаг — релизная приемка,
...
Последний полный прогон после обновления контракта урока: `uv run --locked python
scripts/run_lesson_tests.py` завершился успешно, `Completed lesson test suites: 200;
rubric-verified lessons: 1`. Локально также пройдены 45 root tests и все
