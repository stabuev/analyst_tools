...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `2094f2e` — завершение фазы 04

Локальная `main` опережает `origin/main` на три коммита. Рабочее дерево содержит
незакоммиченную полностью завершённую фазу 05 и проект фазы 06. Push и commit
выполняются только по явной команде пользователя. Перед продолжением проверьте актуальное
...
- 58 завершенных уроков.
- Фазы 00–05 полностью завершены.
...
Фаза 06 полностью спроектирована:

- все 11 уроков переведены в статус `designed`;
...
- новые зависимости не добавлены: версии библиотек будут зафиксированы перед первым
  использующим уроком, без pre-release.

## Следующий содержательный шаг

Разработать `06/01` — **«Вопрос раньше графика»**.

- Сначала создать детерминированный `tiny` profile `user_journeys` и машинный data
  contract, достаточные для первых двух уроков.
- Реализовать visual question brief без зависимости от plotting libraries.
- Артефакт должен принимать вопрос, grain, metric, comparison и decision и выдавать
  машинно-проверяемый план доказательства и rubric выбора представления.
- После завершения `06/01` последовательно разрабатывать `06/02` «Аудит набора данных».
- Matplotlib добавлять в locked environment только перед `06/03`.

Фазы 00–05 завершены. Фаза 06 спроектирована, завершенных уроков в ней пока нет.
