...
В предыдущем коммите переработан урок `03/05` «GroupBy и единица анализа» как
...
В предыдущем коммите переработан урок `03/06` «Joins, ключи и cardinality»
как прямое продолжение order-level свёртки `03/05`. Урок подробно разводит grain обеих
сторон, частоты ключа, четыре вида cardinality, тип `how`, target grain и политику
unmatched-ключей; число строк сначала выводится вручную через произведение частот. Старые
повторное чтение CSV, расчёт `line_total`, `groupby`, `observed` и CLI удалены. Чистый
`merge_with_contract` проверяет обязательные ключи, dtype, уникальность, пересечения
столбцов и row-count prediction, считает уникальные unmatched-ключи независимо от `how`;
`attach_order_rollup` принимает точный nullable-контракт `03/05`, сохраняет order-level
grain и значения заказов и блокирует свёртки неизвестных заказов. Двадцать один
behavioral test покрывает направление cardinality, составной ключ, четыре типа join,
размножение строк, null-ключи, покрытие, сохранение строгого `pd.NA` и ошибки входного
контракта. Дополнительное чтение расширено до десяти проверенных материалов: трёх
русскоязычных официальных руководств Postgres Pro, Microsoft Learn и Yandex Cloud,
шести актуальных страниц pandas 3.0.4 и первичной статьи Tidy Data. Подборка ведёт от
базового merge к проверке частот и дубликатов, переносу в SQL, many-to-many моделям и BI.

В предыдущем коммите переработано основное содержание нынешнего урока `03/08` «Pivot,
melt и tidy data». Урок продолжает модель grain, составного ключа, dtype и агрегации из
`03/01`–`03/06`: объясняет wide/long как расположение фактов, отделяет tidy data от
механического удлинения таблицы, выводит число строк `melt`, строит полный ключ
pivot-ячейки и разводит чистый `pivot` с осмысленной агрегацией. Старые чтение сырого
CSV, очистка статусов, повторный `groupby`, безусловный `fillna(0)` и CLI удалены. Чистые
`to_long` и `pivot_unique` проверяют полный идентификатор, роли и dtype столбцов,
сохранение пропусков, сверку строк и уникальность ячейки; 23 behavioral tests покрывают
round trip, разреженную long-форму, неполный составной ключ, технический индекс,
конфликты имён, пустой типизированный вход и отсутствие мутации. Дополнительное чтение
расширено до десяти проверенных материалов: трёх русскоязычных объяснений и семи
англоязычных официальных или первичных источников. Подборка ведёт от сквозного примера
pandas и модели tidy data к точным контрактам `melt`, `pivot`, `pivot_table`, переносу в
Power Query и подготовке данных для визуализации в seaborn.

В предыдущем коммите переработан нынешний урок `03/09` «Даты, интервалы и часовые
пояса». Он теперь строится вокруг трёх отдельных вопросов: когда произошло событие,
сколько времени прошло и к какому локальному дню оно относится. Урок явно вводит
`Timestamp`, `Timedelta`, `NaT`, aware/naive-время, UTC, offset и именованную timezone;
разводит фиксированную длительность и календарный сдвиг, `tz_localize` и `tz_convert`,
`.dt.seconds` и `total_seconds()`. Контракт входа принимает только ISO 8601 timestamps с
явным offset, не позволяет `utc=True` молча принять naive-время за UTC и отдельно
объясняет числовой Unix time с обязательным `unit`. Чистые функции сохраняют
`Timedelta` до границы отчёта, проверяют label alignment, отрицательную длительность,
пропуски, timezone, индекс и grain; CLI и скрытый `Period` для недель удалены. Тринадцать
behavioral tests покрывают смену локального дня, DST-интервал, 49 часов, ошибки
контракта, отсутствие мутации и dtype результата. Resampling, rolling, частоты и
временные ряды сознательно оставлены фазе 14. Дополнительное чтение расширено до десяти
аннотированных материалов: трёх русскоязычных и семи англоязычных источников о модели
datetime, `Timedelta`, часовых поясах, DST и календарных смещениях.

В коммите `60d80f64` переработан нынешний урок `03/10` «Строки и категориальные
типы».
Он связывает nullable `string` и точные сравнения предыдущих уроков с последствиями для
`groupby`, `merge`, фильтрации и сортировки; подробно разделяет техническую Unicode-
нормализацию, проверку формата, aliases, словарь и политику unknown. Основной минимум
`.str` включает нормализацию, `fullmatch` и один пример `extract`, но не превращается в
курс regex или NLP. Модель `category` теперь объясняет categories, codes, missing с code
`-1`, ordered-порядок, неиспользованные уровни, вывод словаря из текущей порции и риск
потери unknown при прямом приведении к `CategoricalDtype`. Артефакт упрощён до чистых
`normalize_text`, `fullmatch_text` и `categorize_text`: CLI удалён, политика `other`
создаёт стабильный dtype независимо от текущих значений и возвращает audit до
необратимого объединения. Шестнадцать behavioral tests покрывают Unicode, blank →
missing, aliases, индекс, отсутствие мутации, стабильность словаря, evidence и
ordered-сортировку. Время урока увеличено с 75 до 105 минут. Дополнительное чтение
расширено до десяти проверенных материалов: трёх русскоязычных и семи англоязычных;
большинство составляют актуальная официальная документация pandas и Microsoft и
первичный стандарт Unicode.

Урок `03/11` «Method chaining и читаемые пайплайны» переработан как композиционный шаг
между локальными механизмами `03/01`–`03/10` и итоговой витриной `03/12`. Содержание
теперь начинает с обычной композиции через именованные промежуточные таблицы и только
затем вводит `pipe`; разделяет `assign`, доменную стадию, pass-through checkpoint и
осмысленный разрыв цепочки. `query` удалён из outcome как необязательный второй язык
фильтрации, а CLI оставлен следующему уроку об экспорте. Чистый `prepare_orders` строго
различает invalid и missing amount, требует explicit offset, проверяет стабильный словарь
status и не позволяет missing или unknown превратиться в неоплаченный заказ.
Четырнадцать behavioral tests покрывают порядок стадий, grain, dtype, категориальный
словарь, индекс, отсутствие мутации, invalid amount, naive time, unknown status и
локализацию ошибки на ближайшей границе. Дополнительное чтение расширено до десяти
проверенных материалов: трёх русскоязычных и семи англоязычных источников о `pipe`,
`assign`, чистых функциях, Copy-on-Write, тестировании DataFrame и границах длинных
цепочек.

Урок `03/12` «Итоговая витрина: экспорт и проверяемый handoff» переработан как
интеграционный финал Pandas-фазы. Он повторно применяет контракты типов, времени,
категорий, агрегаций, joins и композиционного pipeline из `03/01`–`03/11`, но добавляет
только необходимую новую границу: решение о публикации и приёмку фактически записанного
пакета. Сборщик строго различает invalid и missing, блокирует нарушения grain,
обязательных словарей, явного UTC offset, item coverage и известные расхождения сумм;
разрешённую неполноту сохраняет как warnings с count и examples. Публикация выпускает
детерминированный `order_mart.csv` и `manifest.json` с отдельными версиями manifest и
dataset schema, параметрами, логической схемой, quality report и checksums источников и
артефакта. Отдельная команда `verify` проверяет package со стороны получателя. Двадцать один
behavioral tests покрывают сборку, publish blockers, warnings, детерминированный экспорт,
подмену CSV или manifest и обе CLI-команды. Дополнительное чтение расширено до десяти
проверенных материалов: двух содержательных русскоязычных источников и восьми актуальных
официальных или первичных страниц о pandas I/O, CSV dialect, checksum, metadata,
Data Package, Table Schema и JSON Schema.
После сквозного аудита реализация разделена на `mart_contracts.py`, `mart_pipeline.py`
и `mart_delivery.py`; `mart_builder.py` оставлен тонкой CLI-оболочкой. Путеводитель
теперь требует concat-аудит только при нескольких партиях одной сущности, а урок явно
объясняет, почему publish gate использует более строгую completeness-политику, чем
учебная агрегация в `03/05`.

По результатам связного аудита фазы между joins и reshape добавлен новый урок `03/07`
«Concat, партии данных и порядок строк». Он сразу после `03/06` разводит два способа
объединения: `merge` сопоставляет сущности по ключу и добавляет столбцы, а вертикальный
`concat` добавляет одинаковые по grain наблюдения. Чистый `concat_batches` проверяет
точную схему, dtype, обязательный бизнес-ключ, provenance, сумму строк и пересечение
ключей внутри и между партиями; результат получает свежий `RangeIndex` и полный
детерминированный порядок с бизнес-ключом как tie-breaker. CLI, горизонтальный concat,
`MultiIndex` и выбор версии дубликата сознательно не добавлены. Прежние уроки `03/07`–
`03/11` сдвинуты на `03/08`–`03/12`, зависимость фазы 04 обновлена. Фаза содержит 12
уроков ровно на 18 часов. Сквозной путеводитель переписан: порядок изучения отделён от
порядка исполнения, нормализация строк поставлена до точной фильтрации, а reshape показан
как ветка отчёта. Селектор `03/03` теперь принимает как nullable string, так и
категориальные столбцы со строковыми уровнями и сохраняет missing-семантику.

Урок `04/01` «Grain, ключи и связи» переработан как понятийный мост между Pandas и
реляционной моделью. Он теперь опирается на уже освоенные grain, кандидатные и составные
ключи, cardinality и unmatched-строки, а новым результатом делает выбор primary key,
направление и обязательность foreign key, различие missing-ссылки и orphan-ключа и
границу между объявленным, наблюдаемым и обеспечиваемым ограничением. Grain `events`
уточнён до одного логического события. Прежний 436-строчный Python/DuckDB CLI удалён:
`GROUP BY`, `HAVING`, anti join, CTE и Python DB API больше не используются до
соответствующих уроков `04/04`–`04/06` и `04/11`. Практика переведена в
`guided-artifact`: студент заполняет самостоятельный шаблон реляционного контракта и
проверяет его по отдельной scenario-based рубрике. Дополнительное чтение расширено до
десяти проверенных материалов: три русскоязычных и семь англоязычных источников ведут от
grain и ограничений к границам CSV, BI-связям, проверкам данных и машиночитаемым
контрактам без преждевременного обучения SQL-синтаксису.

Урок `04/02` «SELECT и выражения» переработан как первый исполняемый SQL-шаг после
реляционного контракта `04/01`. Он подробно переносит знакомые выбор столбцов, фильтрацию,
производные поля и порядок из Pandas в логические стадии `FROM -> WHERE -> SELECT ->
ORDER BY`; студент вручную строит полную таблицу результата, а затем сверяет grain,
колонки, типы `DECIMAL`, значения `CASE` и tie-breaker. Python CLI, parameter binding,
SQL injection и connection lifecycle удалены до `04/11`, а `NULL`/`UNKNOWN` оставлены
следующему `04/03` с одним явным входным предположением. Основной артефакт теперь
`paid_rub_orders.sql` с отдельным `select_result_contract.json`; девять behavioral tests
проверяют ручной oracle, схему и типы, точную CASE-границу, десятичную арифметику,
детерминированный порядок, schema drift, нарушение grain и missing amount выбранной
популяции. Время урока увеличено с 75 до 90 минут. Дополнительное чтение расширено до
десяти точечно аннотированных материалов: трёх русскоязычных и семи англоязычных,
преимущественно из официальной документации; подборка не забегает в `NULL`, агрегаты,
оконные функции, CTE и Python API.

Урок `04/03` «NULL и трёхзначная логика» переработан как прямое продолжение `WHERE` и
`CASE` из `04/02` и nullable-масок из фазы 03. Он теперь человеческим языком разводит
`NULL` в данных и `UNKNOWN` как результат условия, выводит полные правила `NOT`, `AND` и
`OR`, показывает, почему `WHERE` сохраняет только `TRUE`, а `CASE ... ELSE` может
правдоподобно неверно классифицировать отсутствующую сумму. Ручной механизм строит
построчное разбиение `above_threshold / at_or_below_threshold / missing`, после чего
SQL-аудит сохраняет все 12 строк и делает исходный предикат, его отрицание и политику
пропуска наблюдаемыми. Прежние `COUNT`, агрегатный `FILTER`, `GROUP BY`, CTE,
`CROSS JOIN`, `NOT EXISTS` и Python CLI удалены до уроков `04/04`–`04/06` и `04/11`;
денежная сумма снова хранится как `DECIMAL(18, 2)`. Основной артефакт теперь
`null_policy_audit.sql` с отдельным `null_policy_contract.json`; шестнадцать behavioral
tests покрывают полные таблицы истинности, `WHERE`, `CASE`, граничное значение, явный
`COALESCE`, grain, типы и отсутствие скрытой потери строк. Время урока увеличено с 75 до
90 минут; фаза 04 теперь занимает ровно 18 часов. Дополнительное чтение расширено до
десяти проверенных официальных источников: три русскоязычных и семь англоязычных
материалов ведут от таблиц истинности и поведения DuckDB к `CASE`, `COALESCE`,
NULL-safe сравнениям и переносимой семантике `NOT IN`, не забегая в обучение агрегатам,
соединениям и подзапросам.

Урок `04/04` «Агрегации и уровни детализации» переработан как SQL-продолжение pandas
`GroupBy` из `03/05` и NULL-семантики `04/03`. Он теперь подробно объясняет смену grain,
агрегацию всей популяции и по ключам, различия `COUNT(*)`, `COUNT(expression)` и
`COUNT(DISTINCT ...)`, знаменатель `AVG`, судьбу `NULL` в ключе и значении, а также
границы `WHERE`, агрегатного `FILTER` и `HAVING`. Некорректный oracle `5005`, полученный
сложением RUB, USD, EUR и KZT, удалён: денежные значения проверяются только внутри своей
валюты, а полнота оплаченной суммы наблюдаема через отдельные счётчики. Python CLI,
повторное чтение CSV и учебное использование DB API удалены; основной артефакт теперь
`currency_aggregate_audit.sql` с отдельным `currency_aggregate_contract.json`, а Python
служит только reference runner. Семнадцать behavioral tests покрывают ручную таблицу,
grain, типы, reconciliation, частично и полностью неизвестные суммы, `NULL`-группу,
`WHERE` / `FILTER` / `HAVING`, `UNKNOWN` в условии, точный `DECIMAL` и запрет общего
денежного итога. Длительность остаётся 90 минут. Дополнительное чтение расширено до
десяти проверенных официальных источников: три русскоязычных и семь англоязычных
материалов ведут от базового `GROUP BY` к точной NULL-семантике агрегатов, условному
`FILTER`, `HAVING` и переносимому синтаксису, но явно откладывают grouping sets, pivot,
редкие и приближённые агрегаты.

Урок `04/05` «Joins без размножения метрик» переработан как SQL-продолжение связей из
`03/06`, реляционной модели `04/01` и агрегаций `04/04`. Он подробно разделяет grain
обеих сторон, полный join key, multiplicity совпадений, coverage и target grain;
предсказывает число строк через произведение частот и объясняет, почему `LEFT JOIN`
сохраняет unmatched-строку, но не защищает от fanout. Основная практика строит
`item_totals` на grain заказа, затем безопасную order-level витрину с отдельными
состояниями для missing и orphan reference, отсутствующих позиций, неизвестной суммы и
реального расхождения. Некорректные общие суммы `5005` и `7705` удалены: fanout
проверяется отдельно по RUB, USD, EUR и KZT, деньги остаются `DECIMAL`. Урок охватывает
`INNER`, `LEFT`, `SEMI` и `ANTI JOIN`, составной ключ и `NULL` в условии, но оставляет
CTE уроку `04/06`, а Python API — `04/11`. Прежний Python CLI удалён; артефакт состоит
из `item_totals.sql` и `safe_order_mart.sql`, а Python используется только как
reference runner. Девятнадцать behavioral tests покрывают fanout, дубликат правого
ключа, двусторонний coverage, составной ключ, `NULL`, ложное исправление через
`DISTINCT`, состояния сверки, схему, типы и отсутствие будущих механизмов.

Урок `04/06` «CTE, подзапросы и композиция» переработан как прямое продолжение
двух отдельных запросов `04/05`. Он связывает уже знакомые операции в четыре этапа:
`item_totals -> safe_order_mart -> paid_order_mart -> currency_summary`; для каждого
явно фиксирует зависимости, population, grain, ключ и проверку. Отдельно подробно
разведены три формы подзапроса по ожидаемому результату: отношение в `FROM`, одно
значение scalar subquery и существование строки через `EXISTS` / `NOT EXISTS`, включая
коррелированный пример и ловушку `NOT IN` при `NULL`. Итог сохраняет отдельные суммы
RUB, USD, EUR и KZT и показатели качества соединения. Повторные загрузка и типизация,
Python API, CLI, рекурсивные CTE, materialization и `EXPLAIN` удалены из основного
маршрута. Артефакт теперь состоит из `checked_order_pipeline.sql` и компактного
`subquery_audit.sql`; Python оставлен только reference runner. Двадцать behavioral
tests проверяют контракты всех этапов, fanout, пропавшие товары, неполную сумму,
смешение валют, ошибку scalar subquery и NULL-семантику поиска отсутствия.
Дополнительное чтение расширено до десяти проверенных материалов: четырех
русскоязычных и шести англоязычных официальных или первичных источников. Подборка ведет
от точного поведения DuckDB и русскоязычной документации PostgreSQL к практическим
упражнениям, переносимости derived tables, строгому контракту scalar subquery и
прикладному стилю CTE в dbt, не делая рекурсивность и materialization обязательными.

Прежний урок `04/07` разделён на два самостоятельных учебных шага. Первая часть —
«Оконные функции: разделы, порядок и соседние строки» — сохраняет grain заказов,
подробно вводит `PARTITION BY`, оконный и финальный `ORDER BY`,
детерминированный tie-breaker, peers, различия `row_number` / `rank` / `dense_rank`,
`lag` / `lead` и отбор оконного результата через CTE из `04/06`. Основной артефакт
переведен с Python CLI на чистый `order_sequence_windows.sql`; companion-запрос
`latest_paid_order_per_user.sql` показывает переносимый паттерн «окно в CTE -> фильтр».
Двадцать один behavioral test проверяет grain, типы, границы раздела, равные суммы,
одинаковые timestamps, неизвестный `amount` без ложного числового ранга, канонический
словарь `status`, входной контракт и отсутствие оконных агрегатов, frames, загрузки CSV
и Python API в SQL. Дополнительное чтение расширено до десяти официальных источников:
три русскоязычных и семь англоязычных материалов последовательно закрепляют модель
окна, именованные окна, детерминированный порядок, ранги, соседние строки и фильтрацию
оконного результата, не превращая frames второго урока в скрытый пререквизит первого.

Новый `04/08` «Оконные агрегаты и рамки: накопительные и скользящие расчёты» завершает
эту пару. Урок связывает `SUM` / `AVG` / `COUNT` из `04/04` с partition и order из
`04/07`, отдельно вводит frame как третью ось контракта, объясняет полный итог,
накопление, rolling-3 с текущей строкой, prior-3 без неё, обрезание на границе partition,
`NULL`-знаменатели, default frame, peers и различие позиционного `ROWS` и диапазона
значений `RANGE`. Основной артефакт `order_amount_frames.sql` остаётся чистым SQL и
публикует row count и known-value count рядом со средними; companion
`frame_semantics_experiment.sql` делает наблюдаемыми peers, разрыв ключа и ловушку
`last_value`. Python используется только как reference runner без CLI. Двадцать два
behavioral tests проверяют grain, типы, суммы по валютам, границы frames, включение
текущей строки, неизвестный `amount`, одинаковые timestamps, default / `ROWS` / `RANGE`
и входной контракт. Десять аннотированных источников ведут от основной документации
DuckDB и русскоязычной практики к `NULL`-агрегатам, числовым типам, точному frame syntax
и сравнению диалектов. Прежние уроки `04/08`–`04/12` сдвинуты на `04/09`–`04/13`, а
фаза теперь содержит 13 уроков на 1170 минут (19,5 часа) с заявленным диапазоном 15–20
часов.

`04/09` «Время и даты в SQL» переработан как перенос уже знакомой модели времени из
pandas в строгие SQL-типы DuckDB. Урок теперь различает source string, локальный
`TIMESTAMP`, instant `TIMESTAMPTZ`, `DATE` и `INTERVAL`, объясняет два направления
`AT TIME ZONE`, явную business timezone, зависимость отображения от timezone сессии,
календарные границы `date_trunc` / `date_diff`, полуоткрытые периоды и различие
календарного шага и прошедшей длительности на DST. Основной артефакт заменён с Python CLI
на чистый `order_time_model.sql`: он сохраняет grain и временные SQL-типы, требует явный
offset и различает `valid`, `missing` и `invalid`; Python оставлен только reference
runner. Companion `temporal_semantics_experiment.sql` воспроизводит 23-часовой интервал
между соседними местными полуднями при переходе `Europe/Berlin` на DST. Четырнадцать
behavioral tests проверяют типы, offsets, границы бизнес-дня, независимость от timezone
сессии, naive-вход, качество парсинга, календарные месяцы, DST, `date_diff` и
полуоткрытый фильтр. Дополнительное чтение расширено до десяти проверенных материалов:
двух русскоязычных страниц Postgres Pro и восьми англоязычных официальных или первичных
источников. Подборка ведёт от типов и двух направлений `AT TIME ZONE` к календарным
функциям, interval, `TRY_CAST`, promotion casts, полуоткрытым периодам и модели timezone
в материалах W3C и IANA.
...
- 19 фаз.
- 200 уроков в программе.
- 200 завершенных уроков.
...
- суммарная длительность уроков — 1080 минут, или 18 часов;
- каждый урок содержит документацию, executable example, behavioral tests, квиз и
  самостоятельный артефакт;
- суммарно фаза содержит 204 behavioral tests;
- `03/12` реализует интеграционный проект по сборке и передаче order mart;
...
- все 13 уроков получили тип, время, последовательные зависимости, измеримый результат и
  артефакт;
- суммарная длительность фазы — 1230 минут, или 20,5 часа;
- `04/13` назначен интеграционным проектом по сборке проверенных SQL-витрин;
...
- committed `tiny` содержит 50 строк, локальный `sample` — 525 005 строк для анализа
  планов запросов;
- все 13 уроков переведены в статус `complete`;
- исполняемые уроки фазы содержат 208 behavioral tests; `04/01` использует отдельную
  рубрику guided-artifact;
- `04/01`–`04/05` покрывают grain, SELECT, NULL, агрегаты и безопасные JOIN;
- `04/06`–`04/10` покрывают CTE, две части окон, временные зоны и cohort matrix;
- `04/11` поставляет явную проверяемую границу DuckDB ↔ pandas, а `04/12` — сравнение
  `EXPLAIN ANALYZE`;
- `04/13` реализует интеграционную поставку `order_mart.csv`, `user_summary.csv` и
  checksum-manifest с явной границей SQL/Python/pandas, а отдельный phase-exit проверяет
  перенос окон, frames, когорт и plan-аудита без искусственного включения их в mart.
...
защитой. Все 19 фаз и 200 уроков имеют статус `complete`. Текущий содержательный шаг —
...
Все 19 фаз и 200 уроков завершены. Текущий последовательный аудит находится в фазе 05.
Основное содержание, практика, артефакт, тесты и квиз `04/11` «DuckDB из Python»
переработаны как продолжение когортной модели `04/10`: типизированный pandas DataFrame
явно регистрируется в переданной connection, доверенный SQL получает только
параметры-значения, компактный результат проверяется по columns и dtypes, а регистрация
снимается в `finally`. Прежние сырой CSV, денежный `DOUBLE`, универсальный CLI и regex-
обещание read-only удалены. Артефакт `duckdb-dataframe-query` покрыт 18 behavioral tests.
Дополнительное чтение `04/11` доведено до десяти проверенных материалов: трёх
русскоязычных и семи англоязычных официальных или первичных источников о Python API
DuckDB, явном владении соединением, регистрации DataFrame, связывании параметров,
материализации результата и границе между значениями и SQL-структурой. Неработающая
русская ссылка Python `sqlite3` заменена живым официальным материалом YDB, а подборка
дополнена первичной документацией PostgreSQL/libpq.

Урок `04/12` «Планы запросов и стоимость» переработан как доказательный мост между
выполнением DuckDB SQL в `04/11` и выбором границы инструментов в `04/13`. Урок теперь
учит сначала доказывать точную эквивалентность результата, вручную предсказывать крупные
ветви и читать physical plan от scan-листьев к корню. Прежние сырой CSV, `all_varchar`,
текстовые regex по ASCII-плану и CLI удалены: знакомый типизированный DataFrame `04/11`
временно регистрируется в caller-owned connection, а `EXPLAIN (FORMAT JSON)` и
`EXPLAIN (ANALYZE, FORMAT JSON)` сводятся в таблицу `estimated_rows` / `actual_rows` /
`rows_scanned`. `duckdb-plan-audit` блокирует кандидата с другим результатом, сохраняет
SQL, параметры, версию движка и входной scope, снимает relation в `finally` и не выдаёт
один timing за benchmark. Двадцать behavioral tests покрывают две scan-ветви против
одной, estimates vs actual, неизвестную сумму пустой когорты, перенос на join, scope и
cleanup после успеха и ошибки.

Интеграционный урок `04/13` «SQL или DataFrame: выбор инструмента» переработан как
финальная поставка SQL-фазы. Две витрины строятся в DuckDB на grain `order_id` и
`(user_id, currency)` и экспортируются напрямую через `COPY`: полные результаты больше
не материализуются через `fetchall()` или pandas. Revenue сверяется отдельно для RUB,
USD, EUR и KZT без бессмысленного общего итога. Publish gate проверяет ключи источников,
orphan items, сохранение order grain, заполненность paid amount и валюты, денежную сверку
и смену grain независимо от tiny-контролей; разрешённая неполнота остаётся warnings.
Переносимый package включает обе marts, точные SQL assets, evidence выбора границы
инструментов, manifest без абсолютных путей и receiver-side verify с защитой от
tampering, path traversal и undeclared files. Артефакт покрыт 17 behavioral tests,
дополнительное чтение содержит десять проверенных материалов: три русскоязычных и семь
англоязычных официальных или первичных источников.

Содержательная последовательная переработка `04/01`–`04/13` завершена, и фаза 04
полностью закрыта. После отдельного сквозного аудита исправлена старая нумерация мостов
после разделения оконных функций; `04/02` теперь переносит `pandas.concat` в
`UNION ALL` с контрактом схемы, provenance и пересечения ключей; `04/13` требует
компактный phase-exit по окнам, frames, когортам и плану запроса. Длительность `04/08`
увеличена до 120 минут, а фактический контракт фазы синхронизирован на 20,5 часа.

Урок `05/01` «CSV и неоднозначность типов» переработан как смена роли после `04/13`:
вместо поставщика проверенного SQL-пакета студент становится получателем неизвестных
байтов и последовательно доказывает encoding, dialect, структуру и типы. Исправлен
пререквизит, восстановлен мост к dtype/null/timezone-контрактам фаз 03–04. CSV-аудитор
теперь отвергает naive timestamps, неправильную группировку тысяч и не конечные значения
Decimal; точное число строк заменено необязательной политикой `exact` / `min`, а
ограничение глобальной null policy названо явно. Практика покрыта 17 behavioral tests,
квиз содержит два входных и четыре итоговых вопроса с правдоподобными дистракторами.
Дополнительное чтение расширено до десяти проверенных материалов: трёх русскоязычных и
семи англоязычных официальных или первичных источников о CSV dialect, parser inference,
encoding errors, Decimal, aware/naive timestamps, SQL `NULL` и переносимых табличных
метаданных.

Урок `05/02` «Excel как источник: листы, диапазоны и формулы» переработан как следующий
контракт внешней поставки после CSV. Аудитор теперь отделяет workbook, sheet и
`fixed_range`, обнаруживает непредусмотренные строки ниже диапазона, скрытые строки и
столбцы, merged cells внутри данных, неправильные типы и дубликаты бизнес-ключа. Naive
Excel datetime получает явный `source_timezone`, а формулы разрешаются только при
наличии cached values; openpyxl не выдаётся за расчётный движок. Невалидная спецификация
и повреждённая книга завершаются контролируемой ошибкой. Практика покрыта 15 behavioral
tests, квиз содержит два входных и пять итоговых вопросов. Дополнительное чтение
расширено до десяти проверенных официальных или первичных материалов: трёх на русском и
семи на английском.

Урок `05/03` «JSON и вложенные структуры» переработан как переход от табличных файлов к
документу с envelope, родительским массивом событий и дочерним массивом позиций. Урок
теперь последовательно различает missing path, явный `null` и пустой массив, строит
отдельные grains `event_id` и `(event_id, item_position)`, проверяет их фактически, а
также инвентаризирует пути всего документа, включая top-level metadata. Самостоятельный
нормализатор строго требует UTF-8, отвергает duplicate object keys, `NaN` / `Infinity`
и timestamps без offset, разделяет ошибки формы, типов, путей и grain и не публикует
JSONL для невалидного отчёта даже при `--allow-failures`. Raw bytes и SHA-256
сохраняются до преобразований; перед поставкой input сверяется повторно. Практика
покрыта 19 behavioral tests, квиз содержит два входных и пять итоговых вопросов.
Дополнительное чтение расширено до десяти проверенных материалов: трёх на русском и
семи на английском, преимущественно официальных или первичных.

Урок `05/04` «HTTP и Requests» переработан как транспортный мост между сохраненным JSON
из `05/03` и многостраничным клиентом `05/05`. Вместо нераскрытого TCP-жаргона урок
начинается с модели GET request → response и последовательно разводит status, media
type, charset, content encoding, declared `Content-Length` и фактически сохраненные
bytes. Downloader по умолчанию принимает точный `200`, проверяет реальный UTF-8,
ограничивает decoded size, вручную валидирует redirect до следующего запроса и разрешает
HTTP только на loopback. Gzip больше не проваливается из-за сравнения compressed length
с decoded body. Уникальный temp создается рядом с final; неудачное обновление удаляет
только свой temp и сохраняет предыдущий валидный файл. Response-policy failure,
transport/configuration error и успех имеют различимые exit codes. Практика покрыта 25
behavioral tests, включая настоящий локальный HTTP-сервер и CLI; квиз содержит два
входных и пять итоговых вопросов. Дополнительное чтение расширено до десяти проверенных
официальных или первичных материалов: трёх на русском и семи на английском.

Урок `05/05` «Pagination, timeouts и retries» переработан как продолжение строгого
одиночного GET из `05/04`. Клиент следует только договорной same-origin цепочке `next`
до явного `null`, различает attempt и retry, ограничивает повторы на страницу и на весь
обход, учитывает `Retry-After` и jitter и не повторяет постоянные ошибки. Транспортный
контракт сохранён на каждой странице: точный `200`, JSON media type, UTF-8, decoded size,
timeouts и запрет автоматических redirects. Page contract требует `items` и явный
`next`; дубликаты `order_id` внутри и между страницами, cycles, чужой origin и
недоказанное завершение блокируют поставку. Partial records остаются диагностическим
буфером, а self-contained snapshot с records, policy, page/retry journals и checks
атомарно заменяет предыдущий файл только после полного успеха. Практика покрыта 34
behavioral test, квиз содержит два входных и пять итоговых вопросов. Дополнительное
чтение расширено до десяти проверенных официальных или первичных материалов: трёх на
русском и семи на английском.

Урок `05/06` «HTML и Beautiful Soup» переработан как переход от сохранённых HTTP-bytes
к проверяемому табличному grain. Материал теперь явно различает source HTML, parse tree
Beautiful Soup и browser DOM, фиксирует parser и UTF-8 и ограничивает поиск цепочкой
document → container → record → field. Версионированный selector contract объявляет
cardinality, text/attribute source, types, record-count policy и ключ grain. Экстрактор
собирает диагностический отчёт по drift, но атомарно публикует один self-contained
snapshot только после успешных checks; `--allow-failures` влияет лишь на exit code и не
разрешает поставку плохих строк. Практика покрыта 29 behavioral tests, квиз содержит два
входных и пять итоговых вопросов. Дополнительное чтение расширено до десяти проверенных
официальных или первичных материалов: трёх на русском и семи на английском.

Урок `05/07` «Подключение к БД через SQLAlchemy» переработан как перенос уже освоенных
в `04/11` bound values и connection ownership на границу с существующей SQL-БД. Материал
теперь различает DBAPI, driver, dialect, Engine, pool, Connection и Result и объясняет
lazy connection и долгоживущий Engine без повторного обучения SQL. Caller-owned
`read_orders()` не уничтожает Engine; CLI владеет собственным read-only SQLite Engine.
Reviewed SQL вынесен в отдельный asset и использует LEFT JOIN, чтобы не скрывать orphan
orders, а запрос `max_rows + 1` отличает полный срез от незаметного truncation. Строгий
contract версии 2 проверяет source tables, columns, PK, nullability, bind names,
фактические result metadata даже при нуле строк, types, domains, relationship fields и
grain. Только валидный результат атомарно публикуется как self-contained snapshot.
Практика покрыта 30 behavioral tests, квиз содержит два входных и пять итоговых вопросов.
...
Урок `05/09` «Arrow как контракт обмена таблицами» переработан как мост от Parquet-файла
к typed in-memory обмену. Он разделяет семантическую совместимость, metadata fidelity и
memory behavior; объясняет layouts primitive и string buffers и ограничивает zero-copy
вывод конкретным маршрутом и процессом. Самостоятельный CLI принимает внешний строгий
exchange contract, проверяет полные roundtrip-маршруты через pandas ArrowDtype и DuckDB,
закрепляет DuckDB session timezone в UTC, классифицирует разрешённое ослабление field
nullability и не сохраняет process-local addresses. Невалидный результат не публикуется,
а валидный JSON выпускается атомарно с checksum и версиями библиотек. Практика покрыта
30 behavioral tests, квиз содержит два входных и пять итоговых вопросов. Дополнительное
чтение расширено до десяти проверенных официальных или первичных материалов: трёх на
русском и семи на английском.
...
Сквозной аудит фазы 05 подтвердил, что 11 уроков образуют две связанные части: набор
альтернативных source adapters и общий путь validated snapshot → typed storage →
interchange → layout → replayable delivery. В 05/11 добавлена явная карта этой структуры:
API остаётся representative route финального CLI, а CSV, Excel, HTML и БД больше не
выглядят обязательными физическими стадиями одного pipeline. Лёгкий phase-exit использует
уже созданные артефакты и проверяет source boundary, полноту API snapshot, различие
schema/checksum/semantic/workload checks и handoff входа в EDA. Новый код, grader и часы
...
Перед коммитом нового `04/08` точечно пройдены 22 теста самого урока и 41 тест пяти
перенумерованных уроков `04/09`–`04/13`; Ruff, format check, три render check и
`git diff --check` успешны. Из десяти выбранных root-проверок девять успешны;
`test_curriculum_is_valid` останавливается на десяти уже существовавших замечаниях к
формату аннотаций дополнительного чтения неизменённого урока `04/06`. Полный прогон
курса после этого урока не запускался по принятому правилу выполнять его только по
отдельному запросу.

После переработки содержания `04/09` точечно пройдены 14 behavioral tests урока, Ruff
и format check изменённого Python-кода, локальный контракт урока, три render check и
девять связанных root tests. Полный прогон курса не запускался; `git diff --check`
выполняется перед handoff.

После переработки содержания `04/10` прежний Python CLI заменён самостоятельной
SQL-моделью месячной когортной активности над типизированными `users` и `events`.
Урок теперь явно разделяет правила входа и активности, техническую и user-month
дедупликацию, календарный возраст когорты, фиксированный знаменатель, наблюдаемый ноль и
неизвестный будущий период. Горизонт задаётся через `last_complete_activity_month`, а не
через последнее найденное событие. Точечно пройдены 17 behavioral tests, Ruff и format
check, локальный контракт урока, три render check и `git diff --check`; полный прогон
курса не запускался. Дополнительное чтение `04/10` затем расширено до десяти
аннотированных первичных и официальных материалов: три на русском и семь на английском
по определениям когорт, календарным границам, `range` / `LATERAL`, дедупликации,
timezone, production date spine и границам интерпретации retention.

После завершения `04/11` точечно пройдены 18 behavioral tests урока и три render check.
Все десять ссылок дополнительного чтения проверены по сети и отвечают HTTP 200;
локальный контракт аннотаций и `git diff --check` выполняются перед handoff. Полный
прогон курса не запускался.

После переработки `04/12` точечно пройдены 18 behavioral tests, Ruff и format check
изменённого Python-кода, локальный контракт урока, три render check и `git diff --check`.
Все десять аннотированных источников (три на русском и семь на английском) проверены по
сети и отвечают HTTP 200. Полный прогон курса не запускался.

После переработки `04/13` пройдены 17 behavioral tests урока, Ruff, format check,
локальный контракт урока и самостоятельный запуск примера. Все десять ссылок
дополнительного чтения отвечают HTTP 200. Закрывающий прогон 12 executable lessons фазы
04 выполнил 208 тестов без ошибок; guided-практика `04/01` проходит свой структурный
контракт. После форматной правки аннотаций `04/06` полный структурный валидатор, контракт
всей фазы 04 и все 46 root tests проходят без ошибок. Полный lesson runner курса не
запускался.

После сквозного аудита мостов фазы 04 повторно пройдены все 208 behavioral tests её
12 executable lessons и 46 root tests. Точные команды нового phase-exit выполнили
2 теста tie-breaker, 8 тестов frame, 2 теста cohort cutoff и создали самостоятельный
plan report. `validate_course.py`, три render check и `git diff --check` успешны.
Полный lesson runner курса не запускался, поскольку исполняемый код и зависимости
остались неизменными.

После переработки `05/01` пройдены 17 behavioral tests урока, Ruff lint и format check,
проверка воспроизводимости fixtures, `validate_course.py`, проверки актуальности
`outputs/index.json` и статического сайта, а также `git diff --check`. Основной пример
воспроизводит 5 строк, 6 столбцов и сумму `3226.59`. Все десять источников дополнительного
чтения проверены по сети. Полный lesson runner курса не запускался.

После переработки `05/02` пройдены 15 behavioral tests урока, Ruff lint и format check,
46 корневых тестов, `validate_course.py` и три проверки актуальности генераторов.
Самостоятельный пример и CLI воспроизводят пять заказов, сумму `2051.59`, явную
локализацию `Europe/Moscow` и валидный отчёт без failed checks. Все десять источников
дополнительного чтения открыты и проверены по содержанию. Полный lesson runner курса не
запускался.

После переработки `05/03` пройдены 19 behavioral tests урока, Ruff lint и format check,
46 корневых тестов, `validate_course.py` и три проверки актуальности генераторов.
Самостоятельный пример воспроизводит три события, три позиции, оба объявленных grain и
SHA-256 исходных bytes; сгенерированная HTML-страница содержит новый материал. Все
десять источников дополнительного чтения открыты и проверены по содержанию. Полный
lesson runner курса не запускался.

После переработки `05/04` пройдены 25 behavioral tests урока, Ruff lint и format check,
46 корневых тестов, `validate_course.py` и три проверки актуальности генераторов.
Самостоятельный пример и CLI работают через настоящий loopback HTTP-сервер; gzip,
redirects, UTF-8, timeout, size policy, atomic replacement и коды `0` / `1` / `2`
проверены отдельными сценариями. Сгенерированная HTML-страница содержит новый материал,
а все десять источников дополнительного чтения открыты и проверены по содержанию. Полный
lesson runner курса не запускался.

После переработки `05/05` пройдены 34 behavioral tests урока, Ruff lint и format check,
46 корневых тестов, `validate_course.py` и три проверки актуальности генераторов.
Самостоятельный пример воспроизводит три страницы, пять уникальных заказов и явное
завершение по `next=null`; отдельные сценарии проверяют retry budgets, `Retry-After`,
jitter, timeout при streamed read, same-origin chain, page contract, partial-result
failure и атомарную публикацию.
Сгенерированная HTML-страница содержит новый материал, а все десять источников
дополнительного чтения открыты и проверены по содержанию. Полный lesson runner курса не
запускался.

После переработки `05/06` пройдены 29 behavioral tests урока, Ruff lint и format check,
самостоятельный пример и проверка воспроизводимости всех fixtures фазы. Тесты отдельно
доказывают, что selector drift, нарушение grain, типов, encoding и contract schema
блокируют публикацию, а `--allow-failures` не создаёт invalid snapshot. Все десять
источников дополнительного чтения открыты и проверены по содержанию. Структурный
валидатор, 46 root tests, три render check и `git diff --check` успешны. Полный lesson
runner курса не запускался.

После переработки `05/07` пройдены 30 behavioral tests урока, Ruff lint и format check,
самостоятельный DBAPI → SQLAlchemy пример и проверка воспроизводимости fixtures фазы.
Тесты отдельно доказывают caller ownership Engine, возврат Connection в pool, read-only
SQLite URI, binding values, metadata пустого результата, detection `max_rows + 1`,
сохранение orphan order через LEFT JOIN, реальное применение grain и запрет публикации
invalid snapshot. Все десять источников дополнительного чтения открыты и проверены по
содержанию. Структурный валидатор, 46 root tests, три render check и `git diff --check`
успешны. Полный lesson runner курса не запускался.

После переработки `05/09` пройдены 30 behavioral tests урока, Ruff lint и format check,
46 корневых тестов, самостоятельный пример и проверка воспроизводимости fixtures фазы.
Тесты отдельно покрывают полные pandas и DuckDB roundtrip, type и null drift, duplicate
grain, forced copy, строгий contract, bounded input, UTC session state и атомарную
...
После сквозного аудита фазы 05 отдельно пройдены все 319 behavioral tests её 11 уроков и
46 корневых тестов. В финальном уроке добавлены карта альтернативных source adapters и
лёгкий phase-exit на уже созданных результатах; исполняемый код и зависимости не
