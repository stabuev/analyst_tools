...
учит сначала проверять эквивалентность результата, затем читать physical plan снизу
вверх, различать estimates `EXPLAIN` и actual cardinality `EXPLAIN ANALYZE` и
формулировать вывод, ограниченный входом, версией движка и workload. Прежний хрупкий
подсчёт строки `TABLE_SCAN` и сравнение одного tiny timing заменены самостоятельным
`duckdb-plan-report`: он сохраняет SQL, параметры, оба плана и scope, блокирует
однопроходного кандидата с другой population и не закрывает caller-owned connection.
Артефакт покрыт 18 behavioral tests.
