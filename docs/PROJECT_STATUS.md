...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `e588874` — исправления итоговой приемки

Все 201 урок курса завершены. В текущем незакоммиченном этапе улучшается опыт новичка:
добавлены onboarding и подготовительные маршруты, самостоятельный учебный цикл,
phase quiz runner, путеводители по фазам 03 и 07, маршрут временных рядов, независимое
review капстоуна и уточнения сайта, визуальной политики и глоссария. Полный locked-прогон
14 июля 2026 прошёл: structural validator, три render-check, 42 корневых теста и все
201 lesson suites. В lesson runner осталось только стороннее `DeprecationWarning` Ibis
DuckDB backend о переходе с `fetch_arrow_table()` на `to_arrow_table()`.
Перед релизом сайт также получил SEO-контракт: canonical/social metadata, JSON-LD,
sitemap, robots policy, favicon, 404 и статический fallback для 201 урока, маршрутов и
глоссария. GitHub Pages ещё требуется включить и проверить после push.
