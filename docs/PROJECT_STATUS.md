...
**Ветка:** `main`  
**Базовый коммит перед текущим этапом:** `b6a241b` — урок про основы Git
...
После коммита `b6a241b` разработан урок `00/05` «Ветки, pull request и ревью»:

- практический материал по feature-веткам, base, head и merge base;
- локальный сценарий подготовки pull request без зависимости от GitHub API;
- разбор двухточечного и трёхточечного diff;
- CLI `pr_review_packet.py` для проверки commits, файлов, чистоты дерева и PR body;
- аналитический review checklist для Python, SQL и файлов данных;
- тесты реальных временных Git-репозиториев и расхождения base с feature-веткой.

Этот набор составляет текущий завершенный этап проекта.

## Следующий содержательный шаг

Разработать урок `00/06` — **«Секреты и безопасная работа с данными»**.

- Время: 60 минут.
- Результат: студент классифицирует чувствительные данные и использует переменные
  окружения без утечек.
- Артефакт: шаблон безопасной конфигурации проекта.
- Статус в `curriculum.json`: `designed`.

После него перейти к фазе 01.
