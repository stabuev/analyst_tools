...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `cfb81b18` — переработка урока `04/06` о CTE и подзапросах

Все 199 нумерованных уроков курса завершены. Публичный сайт размещён на
...
соответствующих уроков `04/04`–`04/06` и `04/10`. Практика переведена в
...
SQL injection и connection lifecycle удалены до `04/10`, а `NULL`/`UNKNOWN` оставлены
...
`CROSS JOIN`, `NOT EXISTS` и Python CLI удалены до уроков `04/04`–`04/06` и `04/10`;
...
CTE уроку `04/06`, а Python API — `04/10`. Прежний Python CLI удалён; артефакт состоит
...
Урок `04/07` разделяется на два самостоятельных учебных шага. В текущем проходе
переработана первая часть — «Оконные функции: разделы, порядок и соседние строки». Она
сохраняет grain заказов, подробно вводит `PARTITION BY`, оконный и финальный `ORDER BY`,
...
словарь `status`, входной контракт и отсутствие оконных
агрегатов, frames, загрузки CSV и Python API в SQL. Второй самостоятельный урок будет
добавлен следующим проходом и разберет оконные агрегаты, накопительные и скользящие
расчеты, рамки по умолчанию и различие `ROWS` / `RANGE`; последующие уроки пока не
перенумерованы. Дополнительное чтение расширено до десяти официальных источников:
...
- 19 фаз.
- 199 уроков в программе.
- 199 завершенных уроков.
...
- committed `tiny` содержит 50 строк, локальный `sample` — 525 005 строк для анализа
  планов запросов;
- все 12 уроков переведены в статус `complete`;
- исполняемые уроки фазы содержат 143 behavioral tests; `04/01` использует отдельную
...
защитой. Все 19 фаз и 199 уроков имеют статус `complete`. Текущий содержательный шаг —
...
Все 19 фаз и 199 уроков завершены. Следующий содержательный шаг — релизная приемка,
