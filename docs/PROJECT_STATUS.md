...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `b4a79ae5` — переработка урока reshape pandas
...
В предыдущем коммите переработано основное содержание урока `03/07` «Pivot,
...
В текущем незакоммиченном этапе переработан урок `03/08` «Даты, интервалы и часовые
...
временные ряды сознательно оставлены фазе 14.
