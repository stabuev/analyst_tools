...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `f9af6ae` — завершение фазы 03

Локальная `main` опережает `origin/main` на один коммит с завершённой фазой 03. Рабочее
дерево содержит незакоммиченную подготовку фазы 04: детализированную программу, DuckDB,
учебные данные и scaffold первого урока. Push выполняется только по явной команде
...
Фаза 04 подготовлена к разработке:
...
- committed `tiny` содержит 50 строк, локальный `sample` — 525 005 строк для анализа
  планов запросов;
- `04/01` завершён: ручная проверка ключа, DuckDB-аудитор схемы, primary/foreign keys,
  JSON quality gate и девять behavioral tests;
- остальные 11 уроков имеют статус `designed`.

## Следующий содержательный шаг

Разработать урок `04/02` — **«SELECT и выражения»**.

- Статус в `curriculum.json`: `designed`.
- Контракт урока: 75 минут, тип `build`.
- Результат: студент строит проекцию, вычисляемые поля и фильтры и сверяет результат с
  ручной таблицей.
- Артефакт: набор проверяемых `SELECT`-запросов с контрактом результата.

Фазы 00–03 завершены. Фаза 04 спроектирована и готова к последовательной разработке.
