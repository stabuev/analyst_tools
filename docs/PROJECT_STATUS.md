...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `9c129641` — переработанный первый урок pandas
...
В текущем незакоммиченном этапе переработан урок `03/02` «Типы данных и пропуски» и
точечно укреплена его связь с `03/01`. Первый preflight теперь различает pandas-пропуск
и пустой текст в простом или составном бизнес-ключе. Во втором уроке студент объявляет
целевой dtype и `nullable` по столбцу, разделяет допустимый пропуск и непустое invalid-
значение, наблюдает `pd.NA` и готовится к nullable-маскам `03/03`. CLI-аудитор проверяет
`string`, `Int64`, `Float64` и `boolean`, границы `Int64`, конечность `Float64`, сохраняет
отдельные missing/invalid evidence и различает коды `0`, `1`, `2`. Из урока убраны UTC,
категории и память — они оставлены `03/08`, `03/09` и фазе NumPy. Соответствующий участок
путеводителя фазы синхронизирован. Дополнительное чтение перестроено в 10 аннотированных
материалов: восемь актуальных официальных страниц pandas/NumPy и два русскоязычных
разбора Python по двоичной и десятичной арифметике без искусственного заполнения языковой
квоты.
