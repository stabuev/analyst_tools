...
тесты, reference-код, артефакты, прогресс, соседние уроки, canonical и JSON-LD. Sitemap
содержит четыре служебные страницы и 201 страницу урока. Structural validator,
render-check и 43 корневых теста проходят. Пользовательский `site/Archive.zip` не входит
в генерацию и должен оставаться отдельным от изменений курса.
...
Последний полный прогон после разработки `18/07`: `uv run --locked python
scripts/run_lesson_tests.py` завершился успешно, `Completed lesson test suites: 201`.
Локально также пройдены 26 behavioral tests финального урока, 30 root tests и все
