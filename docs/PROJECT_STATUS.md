...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `82aea9ab` — первая часть оконных функций `04/07`
...
- все 13 уроков получили тип, время, последовательные зависимости, измеримый результат и
  артефакт;
- суммарная длительность фазы — 1170 минут, или 19,5 часа;
...
- исполняемые уроки фазы содержат 165 behavioral tests; `04/01` использует отдельную
...
полностью закрыта. Десять содержательных аннотаций дополнительного чтения `04/06`
переформатированы под машинный контракт: полезная часть теперь начинается на строке
ссылки, все источники отвечают HTTP 200, а контракт всей фазы проходит без ошибок.
