...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `f5d534fa` — переработка урока `04/05` о безопасных JOIN
...
- committed `tiny` содержит 50 строк, локальный `sample` — 525 005 строк для анализа
  планов запросов;
- все 12 уроков переведены в статус `complete`;
- исполняемые уроки фазы содержат 89 behavioral tests; `04/01` использует отдельную
