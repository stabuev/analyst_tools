...
**Ветка:** `main`  
**Базовый коммит перед текущим этапом:** `4bbcc64` — урок про терминал и файловую систему
...
После коммита `4bbcc64` разработан урок `00/04` «Git: история аналитического проекта»:

- практический материал по working tree, index, commits и выборочному staging;
- локальный репозиторий из трех сфокусированных commits;
- разбор `.gitignore` и уже tracked-файлов;
- CLI `git_history_check.py` для проверки учебной истории;
- тесты чистого дерева, tracked ignore, сообщений и ширины commits.

Этот набор составляет текущий завершенный этап проекта.

## Следующий содержательный шаг

Разработать урок `00/05` — **«Ветки, pull request и ревью»**.

- Время: 60 минут.
- Результат: студент проводит изменение через ветку и проверяет аналитический код коллеги.
- Артефакт: pull request и чек-лист аналитического ревью.
- Статус в `curriculum.json`: `designed`.

После него завершить урок `00/06`, затем перейти к фазе 01.
