...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `9f09e6a` — завершение фазы 15

Фазы 16–18 разработаны поверх `9f09e6a`: готовы уроки `16/01`–`16/11`,
`17/01`–`17/12` и `18/01`–`18/07`. Все 201 урок курса завершены, полный lesson runner и
structural/render checks пройдены.
...
Фаза 16 завершена уроком `16/11` «Drift, стабильность и interpretation package».
Фаза 17 спроектирована в `docs/phase-17-design.md`; уроки `17/01` «Аналитическая записка
для решения», `17/02` «Excel и XlsxWriter для stakeholder workbook», `17/03`
«Воспроизводимые отчеты с Quarto», `17/04` «HTML, PDF и DOCX как delivery formats»
`17/05` «Интерактивный отчет Plotly» и `17/06` «Приложение на Streamlit» завершены.
Текущий содержательный шаг — разработка урока `17/07` «Кеширование, состояние и свежесть
приложения». Перед коммитом обязательно прогнать полный набор проверок.
