...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `693f6071` — проектный скилл педагогического аудита уроков
...
В текущем незакоммиченном этапе переработан только урок `03/01` «DataFrame, Series и
индексы». Урок теперь связывает позиционную модель NumPy с маркированными осями pandas,
наблюдаемо показывает отношение `DataFrame` и `Series`, точный результат label alignment
и границу между техническим индексом, объявленным grain и кандидатным бизнес-ключом.
Подробный Copy-on-Write и присваивание через `loc` оставлены уроку `03/03`. CLI-инспектор
сокращен до table preflight, требует простой или составной ключ, сохраняет evidence при
провале и различает коды корректного контракта, нарушения данных и невалидного запуска.
Дополнительное чтение перестроено в последовательность из 10 проверенных материалов:
один сильный русскоязычный обзор и девять официальных или первичных англоязычных
источников без искусственного выравнивания языковой квоты.
