...
**Ветка:** `main`
**Базовый коммит перед текущим этапом:** `643ebedb` — переработанный урок про `uv`

Все 197 нумерованных уроков курса завершены. Публичный сайт размещён на
...
В текущем незакоммиченном этапе перерабатывается начало курса по результатам
педагогического аудита. Карта профессий вынесена из программы в ненумерованное введение
`docs/professions-and-routes.md`: она помогает ориентироваться, но не требует теста,
артефакта или преждевременного выбора профессии. Фаза 00 теперь начинается с терминала и
содержит четыре урока: терминал, Git, ветки и ревью, безопасная работа с данными.

Из фазы 01 удалены самостоятельные уроки «Версии Python и совместимость» и
«pyproject.toml как контракт проекта». Минимум про `requires-python`, `.python-version` и
фактический процесс встроен в новый `01/01` про `uv`; секции `pyproject.toml` появляются
дальше just in time — при установке package, настройке Ruff и pytest. Фаза 01 теперь
содержит семь связанных уроков: `uv`, Jupyter kernels, воспроизводимые notebooks, modules
и scripts, Ruff, pytest и CI. Следующая фаза зависит от нового `01/07`.
...
- 19 фаз.
- 197 уроков в программе.
- 197 завершенных уроков.
...
- `02/01`–`02/03`: модель `ndarray`, предсказание форм и аудит `dtype`, диапазонов,
  пропусков и памяти;
- `02/04`–`02/06`: индексация и copy/view, агрегаты по явным осям, затем broadcasting
  и нормализация с применением агрегатов обратно к матрице;
- `02/05` перестроен вокруг предметных осей и grain результата: одна и несколько осей,
  ...
  артефакт поддерживает именованные и отрицательные оси, кортеж осей и покрыт 16 tests;
- `02/06` перестроен как подробный мост от агрегатов к поэлементным операциям:
...
- `02/07` перестроен вокруг локального `numpy.random.Generator`, различия seed/state/stream,
...
- `02/08` перестроен как мост от модели массива к осознанной оптимизации: предметная ось
  ...
  ограниченный вывод с environment metadata; артефакт покрыт 17 tests;
- `02/09` перестроен как итоговый численный контракт фазы: выбор точного или
...
защитой. Все 19 фаз и 197 уроков имеют статус `complete`. Текущий содержательный шаг —
...
Все 19 фаз и 197 уроков завершены. Следующий содержательный шаг — релизная приемка,
