...
## 3. Маска — именованное правило с тремя исходами

На этом шаге используйте уже проверенные значения из предыдущего раздела. Не прячьте
повторное преобразование dtype или нормализацию текста внутрь фильтра: иначе станет
неясно, какая стадия изменила значение и почему строка попала в результат.

Сначала соберите рабочую таблицу и назовите каждое условие:

```python
working = orders.copy()
working["amount_numeric"] = amount_numeric

status = working["status"].astype("string")
is_exact_paid = status.eq("paid")
has_amount = working["amount_numeric"].notna()
is_positive = working["amount_numeric"].gt(0)

mask = is_exact_paid & is_positive
```

Здесь `status` сравнивается с точным значением `paid`. Систематическая нормализация строк
и категорий относится к `03/09`; этот шаг изолирует именно правила выбора.

Проверьте каждое условие и составную маску отдельно:

```python
print(is_exact_paid.value_counts(dropna=False))
print(has_amount.value_counts(dropna=False))
print(is_positive.value_counts(dropna=False))
print(mask.value_counts(dropna=False))
```

`has_amount` остаётся отдельной диагностикой полноты. Мы не включаем его в основное
условие: выражение `False & pd.NA` превратило бы неизвестный результат сравнения суммы в
обычный отказ и смешало бы качество данных с предметным правилом `amount > 0`.

До выборки зафиксируйте количество `True`, `False` и `NA` и примеры меток unknown-строк.
Затем назовите политику: исключить unknown, остановить расчёт или отправить такие строки
в отдельный разбор. Только после этого разрешите маску и одновременно выберите нужные
столбцы:

```python
unknown_rows = working.index[mask.isna()].tolist()
resolved = mask.fillna(False)
paid = working.loc[
    resolved,
    ["order_id", "status", "amount_numeric"],
].copy()
```

Маска pandas связана со строками индексом, а не только длиной. В самостоятельной функции
проверяйте, что маска является булевой `Series`, её индекс уникален и в точности совпадает
с индексом таблицы. Для изменения используйте одно присваивание
`working.loc[resolved, "review_status"] = "review"`, а не цепочку
`working[resolved]["review_status"] = ...`.
