    ...
    },
    {
      ...
      "lessons": [
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 75,
          "outcome": "Оценивает память массива, выбирает dtype по будущему диапазону, пропускам и точности и проверяет преобразование.",
          "artifact": "Проверяемый dtype-контракт для диапазона, пропусков, точности и памяти",
          "type": "build",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 60,
          "outcome": "Проектирует предметные оси случайного эксперимента, использует локальный Generator, фиксирует seed и способ получения чисел и проверяет форму, тип, память и повторяемость симуляции.",
          "artifact": "Воспроизводимый симулятор бинарных событий",
          "type": "build",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 75,
          "outcome": "Переносит поэлементный расчет из Python-цикла в операции NumPy, проверяет построчную и итоговую эквивалентность, сравнивает повторные времена и объясняет компромисс скорости, памяти и понятности.",
          "artifact": "Проверяемый benchmark цикла и NumPy-расчета",
          "type": "case",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 60,
          "outcome": "Формулирует численный контракт результата: выбирает точное или приближенное сравнение, обосновывает rtol и atol из единиц и масштаба, контролирует shape, dtype, диапазон и конечность и диагностирует переполнение, underflow и ошибку накопления.",
          "artifact": "Численный quality gate с диагностикой расхождений",
          "type": "case",
          "tracks": [
            "core"
          ],
      ...
      "lessons": [
        {
          ...
          "status": "complete",
          "time_minutes": 75,
          "outcome": "Различает DataFrame, Series и их маркированные оси, предсказывает выравнивание Series по меткам и проверяет объявленный бизнес-ключ отдельно от технического индекса.",
          "artifact": "CLI-инспектор структуры таблицы и целостности объявленного бизнес-ключа",
          "type": "learn",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 90,
          "outcome": "Объявляет ожидаемый dtype и допустимость пропуска для каждого столбца, строго преобразует значения и отличает допустимый пропуск от непустого неразбираемого значения.",
          "artifact": "Аудитор dtype-контракта, пропусков и неразбираемых значений",
          "type": "build",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "title": "Производные столбцы: арифметика, where и mask",
          "status": "complete",
          "time_minutes": 90,
          "outcome": "На типизированной таблице строит производные столбцы операциями над Series, сохраняет grain и индекс, явно обрабатывает NA в условии и объясняет, почему построчный apply здесь не нужен.",
          "artifact": "Чистая функция производных признаков товарных позиций с явной политикой NA",
          "type": "build",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 105,
          "outcome": "На типизированной таблице товарных позиций объявляет входной и выходной grain, выбирает агрегаты по смыслу метрик, сохраняет неизвестность неполной суммы и сверяет результат с ручным контрольным расчетом.",
          "artifact": "Чистая функция свертки товарных позиций до уровня заказа с grain- и completeness-контрактом",
          "type": "case",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 105,
          "outcome": "Строго переводит timestamps с явными offsets в UTC, вычисляет Timedelta между согласованными моментами и выводит локальный календарный день в заданной бизнес-зоне.",
        ...
        },
        {
          ...
          "title": "Итоговая витрина: экспорт и проверяемый handoff",
          "status": "complete",
          "time_minutes": 105,
          "outcome": "Собирает из нескольких грязных таблиц проверенную order mart, отделяет блокеры публикации от допустимых предупреждений и передает воспроизводимый CSV-пакет с версионированным manifest и независимой проверкой получателем.",
          "artifact": "CLI сборки и независимой проверки versioned delivery package",
          "type": "case",
          "tracks": [
            "core"
          ],
    ...
    },
    {
      ...
      "lessons": [
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 90,
          "outcome": "По бизнес-правилу строит SELECT с явной проекцией, вычисляемыми и условными полями, фильтром и детерминированным порядком, затем сверяет grain, схему, типы и значения с ручной таблицей.",
          "artifact": "Проверяемый SQL SELECT с машинно-читаемым контрактом результата",
          "type": "build",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 90,
          "outcome": "Для сравнений и составных условий предсказывает TRUE, FALSE и UNKNOWN, объясняет поведение WHERE и CASE и задаёт явную политику NULL без подмены неизвестного нулём.",
          "artifact": "Построчный SQL-аудит NULL-политики с машинно-читаемым контрактом",
          "type": "learn",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "title": "CTE, подзапросы и композиция",
          "status": "complete",
          "time_minutes": 75,
          "outcome": "Собирает знакомые SQL-преобразования в цепочку нерекурсивных CTE, объявляет grain и population каждого этапа и выбирает форму подзапроса по ожидаемой форме результата.",
          "artifact": "Проверяемая SQL-цепочка от строк товаров до итогов по валютам",
          "type": "build",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "title": "Оконные функции: разделы, порядок и соседние строки",
          "status": "complete",
          "time_minutes": 90,
          "outcome": "Сохраняет grain заказов, задает независимые разделы и детерминированный порядок строк, различает row_number, rank и dense_rank, получает соседние строки через lag и lead и отбирает результат окна через CTE.",
          "artifact": "Проверяемая SQL-последовательность заказов с нумерацией, рангами и соседними строками",
          "type": "build",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 90,
          "outcome": "Преобразует строковые timestamps в однозначные моменты, строит типизированные бизнес-даты и месяцы в явной timezone и различает прошедшую длительность и календарное расстояние.",
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 105,
          "outcome": "Строит месячную когортную матрицу активности с явными правилами входа, активности, календаря, горизонта наблюдения и фиксированным знаменателем.",
          "artifact": "SQL-модель месячной когортной активности с полным наблюдаемым grid",
          "type": "case",
          "tracks": [
            "core"
          ],
        ...
        },
        {
          ...
          "status": "complete",
          "time_minutes": 90,
          "outcome": "Для двух эквивалентных SELECT читает структурированные EXPLAIN и EXPLAIN ANALYZE снизу вверх, сопоставляет оценки с фактическими строками и находит повторное чтение или неожиданный рост данных без вывода о скорости по одному запуску.",
          "artifact": "Функция проверки эквивалентности и структурированного сравнения планов DuckDB",
          "type": "learn",
          "tracks": [
            "core"
          ],
